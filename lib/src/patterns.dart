...
class WhatsAppPatterns {
  ...
  static const String _endOfLine = r"(\n|\r|\r\n|$)";

  /// Universal
  static const String _universal = _startOfLine + _date + r"(,\s?|\s|،\s?)" + _time + r"(\]\s?-?|:|\s-)\s" + _sender + r":\s" + _content + _endOfLine;

  /// Time first, then Date
  static const String _timeFirst = _startOfLine + _time + r"(,\s?|\s|،\s?)" + _date + r"(\]\s?-?|:|\s-)\s" + _sender + r":\s" + _content + _endOfLine;
  ...
  static const String _systemTimeFirst = _startOfLine + _time + r"(,\s?|\s)" + _date + r"(\]\s?-?|:|\s-)\s" + r"(" + _sender + r":\s" + r")?" + _content + _endOfLine;
  ...
  static const String _iosAttachmentPattern = r"^(?<name>[^\n|\r|\r\n]+)\s?<(attached|pièce jointe|‎прикреплено|‏پیوستہ,|allegato|anexo|adjunto|‎επισυνάφθηκε|‎додано|Anhang|attached|‎附件|附件|‎附件|đính kèm|eklendibifogat|príloha|załączono|atașare|anexado|bijgevoegd|vedlagt|dilampir|‎संलग्न केले|첨부됨|‎添付ファイル|csatolva|terlampir|priloženo|‎अटैच किया गया|‎જોડાયેલ|ceangailte|liite|‏پیوست|vedhæftet|‎επισυνάφθηκε|příloha|adjunt|‎แนบ|‏מצורף|angehängt): (?<file>.*)>" + _endOfLine; 
  ...
  static String _attachmentPattern = "($_iosAttachmentPattern|$_androidAttachmentPattern)";
  ...
  static RegExp _foursquareLocationRegex = RegExp(_foursquareLocationPattern, multiLine: true);
  static String _foursquareLocationPattern = r"^(?<place>.*\(?.*\)?):?\s(?<link>https://foursquare.com.*)\b" + _endOfLine;
  ...
  static String nullMessage = r"^null" + _endOfLine;
  static bool isNullMessage(String message) => _hasMatch(message, nullMessage, caseSensitive: true);
