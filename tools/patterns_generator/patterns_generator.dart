...
final iosDirectory =
    "./tools/patterns_generator/data/iOS Localizables/"; // Directory.current.path
...
final androidDirectory = "./tools/patterns_generator/data/Android Strings/";
...
Future<void> collectAll() async {
  ...
  pattern =
      await collectOneOrMany(["_i3PG"], append: encryptedValues, end: r").?");
  print("ENCRYPTED\n$pattern\n");
  ...
  pattern =
      await collectOneOrMany(["GHe7G", "S?EA5"], append: youDeleted.values);
  ...
  pattern = await collectOneOrMany(["A),w/", "f/]I7"],
      append: missedVoiceCall.values);
  ...
  /// Image omitted - `n*sq0`
  pattern = await collectOneOrMany(["n*sq0"],
      start: r"^\<?(", end: r")\>?.?" + endOfLine);
  print("IMAGE OMITTED\n$pattern\n");

  /// Video omitted - `I4|vp`
  pattern = await collectOneOrMany(["I4|vp"],
      start: r"^\<?(", end: r")\>?.?" + endOfLine);
  print("VIDEO OMITTED\n$pattern\n");

  /// Audio omitted - `a86Wu`
  pattern = await collectOneOrMany(["a86Wu"],
      start: r"^\<?(", end: r")\>?.?" + endOfLine);
  print("AUDIO OMITTED\n$pattern\n");

  /// Sticker omitted - `2K_mB`
  pattern = await collectOneOrMany(["2K_mB"],
      start: r"^\<?(", end: r")\>?.?" + endOfLine);
  print("STICKER OMITTED\n$pattern\n");

  /// GIF omitted - `~.KLq`
  pattern = await collectOneOrMany(["~.KLq"],
      start: r"^\<?(", end: r")\>?.?" + endOfLine);
  print("GIF OMITTED\n$pattern\n");

  /// Contact card omitted - `Sk+0p`
  pattern = await collectOneOrMany(["Sk+0p"],
      start: r"^\<?(", end: r")\>?.?" + endOfLine);
  print("CONTACT CARD OMITTED\n$pattern\n");

  /// Document omitted - `StaLC`
  pattern = await collectOneOrMany(["StaLC"],
      start: r"^\<?(",
      end: r")\>?.?" + endOfLine); // sometimes appear after the file name
  ...
  securityPattern = securityPattern.substring(
      0, securityPattern.length - 1); // remove dot at the end
  ...
  final mediaOmittedValues =
      await getAndroidTranlsationsFor("email_media_omitted");
  final mediaOmitted = mediaOmittedValues.values.map((e) {
    e = e.substring(1, e.length - 1); // <Media omitted>
    e = e.replaceAll("/",
        r"\/"); // some of rtl languages has unescaped backslash in translation
    return e;
  });
  final mediaOmittedPattern = escape(
      r"^<\s?(Media omitted|" + mediaOmitted.join("|") + r")\s?>" + endOfLine);
  print("MEDIA OMITTED (ANDROID)\n$mediaOmittedPattern\n");

  final attachedValues = await getAndroidTranlsationsFor("email_file_attached");
  final attached = attachedValues.values
      .map((e) => e.substring(4, e.length - 1)); // %s (file attached)
  final attachedPattern = escape(r"^(?<file>.*)\s\((file attached|" +
      attached.join("|") +
      r")\)" +
      endOfLine);
  ...
  final locationValues =
      await getAndroidTranlsationsFor("email_location_message");
  final location = locationValues.values
      .map((e) => e.substring(0, e.length - 4)); // location: %s
  final locationPattern = escape(r"^(location|" +
      location.join("|") +
      r"):?\s(?<link>https://maps.google.com/\?q=(?<longitude>[0-9\.]*),(?<latitude>[0-9\.]*))" +
      endOfLine);
  print("LOCATION (ANDROID)\n$locationPattern\n");
  // TODO: For iOS add additionally `[^\s]+`

  final liveLocation =
      await getAndroidTranlsationsFor("email_live_location_message");
  final liveLocationPattern = escape(r"^(live location shared|" +
      liveLocation.values.join("|") +
      r")" +
      endOfLine);
...
Future<String> collectOneOrMany(List<String> keys,
    {Iterable<String> append = const [], String? start, String? end}) async {
  ...
  final string =
      "${start != null ? start : "^("}$joined${end != null ? end : ").?$endOfLine"}";
